...
const [sourceRoot, artifactRoot, evidenceRoot] = process.argv
  .slice(2)
...
const { inspectPaidInstall, inspectPaidStatus, verifyStoredPaidClock } = await sourceModule(
  'xdoc-cli-paid-probe-auth.mjs',
)
...
try {
  ...
  const expected = {
    config,
    payload: JSON.parse(readFileSync(license)).payload,
    licenseSha256: digest(license),
  }
  stage = 'install-inspection'
  const installed = inspectPaidInstall(
    runProbeCandidate({ binary, cwd: work, config, args: ['auth', 'install', license] }),
    expected,
  )
  stage = 'installed-clock'
  verifyStoredPaidClock(config, installed.lastSeenUnixMs)
  stage = 'status-inspection'
  const status = inspectPaidStatus(
    runProbeCandidate({ binary, cwd: work, config, args: ['auth', 'status'] }),
    { ...expected, previousAnchor: installed.lastSeenUnixMs },
  )
  stage = 'status-clock'
  ...
  console.log(
    JSON.stringify({ stage, passed: false, authAssertionLine: line ? Number(line[1]) : null }),
  )
  process.exitCode = 1
} finally {
